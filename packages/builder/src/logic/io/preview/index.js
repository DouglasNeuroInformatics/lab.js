...
const linkStatic = async (cache, [path, data], previewPath) => {
  ...
  const response =
    (await cache.match(`/api/_defaultStatic/${path}`)) ??
    (await fetch(`/api/_defaultStatic/${path}`))

  await cache.put(new Request(`/api/${previewPath}/${path}`), response)
}
...
export const populateCache = async (
  ...
  try {
    ...
    await Promise.all([
      ...Object.entries(study.files).map(file =>
        putFile(cache, file, previewPath),
      ),
