import { fromPairs, zip, pick, pickBy, groupBy, identity } from 'lodash'
...
const processFiles = files =>
  fromPairs(files.map(f => [f.localPath.trim(), f.poolPath.trim()]))
...
const processHooks = hooks =>
  fromPairs(
    hooks
      ...
      .map(h => [h.message, adaptiveFunction(h.code)]),
  )
...
const processNode = node => {
  ...
  const filterOptions = (value, key) =>
    value !== '' && !(key.startsWith('_') || filteredOptions.includes(key))

  const output = Object.assign({}, pickBy(node, filterOptions), {
    content: processContent(node.type, node.content),
    files: node.files ? processFiles(node.files) : {},
    hooks: node.hooks ? processHooks(node.hooks) : node.hooks,
    parameters: node.parameters ? processParameters(node.parameters) : {},
    items: node.items ? processItems(node.items) : null,
    ...
    if (currentNode.children) {
      switch (currentNode.type) {
        ...
        case 'lab.html.Frame':
          ...
          output.content = currentNode.children.map(c =>
            makeComponentTree(data, c),
          )
          break
        case 'lab.flow.Loop':
          // A loop has a template array
          output.template = currentNode.children.map(c =>
            makeComponentTree(data, c),
          )
          break
        default:
          console.log(
            `Unexpectedly found nested components on ${currentNode.type}`,
          )
          break
      }
